...
export default function Dashboard() {
  ...
  const shareOnWhatsApp = () => {
    ...
    const text = `*HealthAI Report — ${patientData?.name}*\n\n` +
      `Age: ${patientData?.age} | Gender: ${patientData?.gender} | Blood: ${patientData?.bloodGroup}\n` +
      `Symptoms: ${patientData?.symptoms?.slice(0, 100)}...\n\n` +
      `*Urgency:* ${data.urgencyLevel}\n\n` +
      `*Possible Conditions:*\n${top3}\n\n` +
      `_Generated by HealthAI — AI-powered health assessment_`;
        ...
        <div style={{ maxWidth: '900px', margin: '0 auto', position: 'relative' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              ...
              <button onClick={shareOnWhatsApp} style={headerBtn('#25D366', '#fff', false)}>📲 WhatsApp</button>
            </div>
          </div>
        </div>
      </div>
